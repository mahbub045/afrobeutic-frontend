        ...
        {isLoading ? (
          ...
          <div className="max-h-[340px] space-y-4 overflow-y-auto pr-2 pb-4">
            {salons?.map((salon: SalonProps) => (
              <div
                key={salon.uid}
                className="bg-card hover:bg-accent/50 flex items-center gap-3 rounded-lg border p-3 shadow-md transition-colors dark:shadow-gray-600"
              >
                <div className="bg-primary/10 flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full">
                  {salon.logo ? (
                    <Image
                      src={salon.logo}
                    ...
                    <div className="text-primary flex h-12 w-12 items-center justify-center text-sm font-medium">
