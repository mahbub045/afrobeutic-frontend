...
const EditBookingServicesModal: React.FC<Props> = ({
  isOpen,
  onOpenChange,
  bookingData,
}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(err);
      Swal.fire({ icon: "error", title: "Failed to update services" });
    }
  };
                          ...
                          {filteredServices.map((service: Service) => (
                            <li key={service.uid}>
                              <label className="my-1 flex w-full cursor-pointer items-center gap-2 px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-800">
                                <input
                                  type="checkbox"
                                  checked={localSelection.includes(service.uid)}
                                  onChange={(e) => {
                                    setLocalSelection((prev) =>
                                      e.target.checked
                                        ? [...prev, service.uid]
                                        : prev.filter((s) => s !== service.uid),
                                    );
                                  }}
