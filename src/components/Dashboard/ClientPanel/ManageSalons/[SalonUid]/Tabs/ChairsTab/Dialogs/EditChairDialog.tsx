...
import { useEditChairMutation } from "@/Redux/Reducers/ClientPanel/ManageSalons/Chairs/ChairsApi";
...
const EditChairDialog: React.FC<ChairDialogsProps> = ({
  isOpen,
  onClose,
  selectedChairData,
}) => {
  ...
  const handleEditChair = async (
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `Successfully edited <b class="text-primary">${values.name}</b> chair.`,
      ...
      console.error("Failed to edit chair:", error);
      toast.error("Failed to edit chair. Please try again.");
    }
    onClose();
  };
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        ...
        <Formik
          initialValues={
            {
              name: selectedChairData?.name || "",
              type: selectedChairData?.type || "",
              status: selectedChairData?.status || "",
            } as ChairFormValues
          }
          validationSchema={ChairSchema}
          enableReinitialize
          onSubmit={handleEditChair}
        >
...
export default EditChairDialog;
